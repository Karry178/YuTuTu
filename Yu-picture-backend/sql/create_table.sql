...
(
    ...
    userRole     varchar(512)                       not null comment '用户角色:user/admin',
...
-- picture表新增审核相关字段，同时优化索引设计提高查询性能
ALTER TABLE picture
    -- 添加新列
    ADD COLUMN reviewStatus  INT DEFAULT 0 NOT NULL COMMENT '审核状态：0-待审核；1-审核通过；2-拒绝',
...
(
    ...
    totalSize  bigint   default 0                 null comment '当前空间下的图片总大小',
    ...
    index idx_spaceLevel (spaceLevel) -- 提升基于空间等级的查询效率
...
-- 创建索引
create index inx_spaceId on picture (spaceId);
